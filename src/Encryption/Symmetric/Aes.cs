...
namespace Kybus.Enigma.Encryption.Symmetric
{
    public sealed class Aes : EncryptionBase
    {
        ...
        private void SetKey(byte[] key)
        {
            ...
            _key = key;
            Mode = (KeyLength)(8 * key.Length);

            _rounds = (Key.Length / 4) + 6; // Only invalid in case the key length is 224 bits but AES only supports 128/192/256
        ...
        public static Aes CreateWithRandomKey(int lengthInBits)
        {
            ...
            byte[] arr = new byte[lengthInBits / 8];
        ...
        {
            int blocks = (int)Math.Ceiling(plainText.Length / 16.0);
            byte[] output = new byte[blocks * 16];
            byte[] lastBlock = null;

            for (int i = 0; i < blocks; i++)
            {
                byte[] cbcVector = CbcModeEnabled ? i == 0 ? CbcInitVector : lastBlock : null;
                ...
                byte[] cipherText = EncryptBlock(block, cbcVector);
        ...
        {
            int blocks = (int)Math.Ceiling(encryptedData.Length / 16.0);
            byte[] output = new byte[blocks * 16];
            byte[] lastBlock = null;
            ...
            for (int i = 0; i < blocks; i++)
            {
                ...
                byte[] cipherText = DecryptBlock(block, cbcVector);
                for (int j = start; j < end; j++)
                {
                    output[j] = cipherText[j - start];
                }
        ...
        private byte[] EncryptBlock(byte[] data, byte[] cbcVector)
        {
            if (CbcModeEnabled)
            {
                data.ApplyXorTo(cbcVector);
            }

            byte[,] state = data.ConvertArrayToState();
            ...
            byte[] encryptedData = state.ConvertStateToByteArray();
            return encryptedData;
        }

        private byte[] DecryptBlock(byte[] encryptedData, byte[] cbcVector)
        {
            byte[,] state = encryptedData.ConvertArrayToState();
            ...
            byte[] decryptedData = state.ConvertStateToByteArray();

            if (CbcModeEnabled)
            {
                decryptedData.ApplyXorTo(cbcVector);
            }
        ...
        private void AddRoundKey(byte[,] state, int round)
        {
            for (int c = 0; c < 4; c++)
            {
                state[c, 0] ^= ExpandedKey[(16 * round) + (4 * c) + 0];
                state[c, 1] ^= ExpandedKey[(16 * round) + (4 * c) + 1];
                state[c, 2] ^= ExpandedKey[(16 * round) + (4 * c) + 2];
                state[c, 3] ^= ExpandedKey[(16 * round) + (4 * c) + 3];
            }
        }
        ...
        {
            byte[,] tempState = state.DeepCopyStateArray();
        ...
        private static void MixColumns(byte[,] state)
        {
            ...
            for (int x = 0; x < 4; x++)
            {
                state[x, 0] = (byte)(BitwiseMultiplication(0x2, tempState[x, 0]) ^ BitwiseMultiplication(0x3, tempState[x, 1]) ^ tempState[x, 2] ^ tempState[x, 3]);
                state[x, 1] = (byte)(tempState[x, 0] ^ BitwiseMultiplication(0x2, tempState[x, 1]) ^ BitwiseMultiplication(0x3, tempState[x, 2]) ^ tempState[x, 3]);
                state[x, 2] = (byte)(tempState[x, 0] ^ tempState[x, 1] ^ BitwiseMultiplication(0x2, tempState[x, 2]) ^ BitwiseMultiplication(0x3, tempState[x, 3]));
                state[x, 3] = (byte)(BitwiseMultiplication(0x3, tempState[x, 0]) ^ tempState[x, 1] ^ tempState[x, 2] ^ BitwiseMultiplication(0x2, tempState[x, 3]));
            }
        }
        ...
        private static void InvMixColumns(byte[,] state)
        {
            ...
            for (int x = 0; x < 4; x++)
            {
                state[x, 0] = (byte)(BitwiseMultiplication(0x0e, tempState[x, 0]) ^
                                          BitwiseMultiplication(0x0b, tempState[x, 1]) ^
                                          BitwiseMultiplication(0x0d, tempState[x, 2]) ^
                                          BitwiseMultiplication(0x09, tempState[x, 3]));
                state[x, 1] = (byte)(BitwiseMultiplication(0x09, tempState[x, 0]) ^
                                          BitwiseMultiplication(0x0e, tempState[x, 1]) ^
                                          BitwiseMultiplication(0x0b, tempState[x, 2]) ^
                                          BitwiseMultiplication(0x0d, tempState[x, 3]));
                state[x, 2] = (byte)(BitwiseMultiplication(0x0d, tempState[x, 0]) ^
                                          BitwiseMultiplication(0x09, tempState[x, 1]) ^
                                          BitwiseMultiplication(0x0e, tempState[x, 2]) ^
                                          BitwiseMultiplication(0x0b, tempState[x, 3]));
                state[x, 3] = (byte)(BitwiseMultiplication(0x0b, tempState[x, 0]) ^
                                          BitwiseMultiplication(0x0d, tempState[x, 1]) ^
                                          BitwiseMultiplication(0x09, tempState[x, 2]) ^
                                          BitwiseMultiplication(0x0e, tempState[x, 3]));
            }
        }
        ...
        {
            byte[] words = new byte[4 * 4 * (_rounds + 1)];
        ...
        private byte[] Rcon(byte i)
        {
            ...
            else if (i > 1)
            {
                ...
                while (i - 1 > 0)
                {
                    _rcon[0] = BitwiseMultiplication(_rcon[0], 0x02);
                    i--;
                }
            }
