...
namespace KybusEnigma.Lib.Padding
{
    public static class LengthPadding
    {
        ...
        {
            var currentLength = buffer.GetLongLength(0);
            var newArrayLength = CalcNewArrayLength(currentLength, blockSize, lengthSize); // Excluding appended length bytes

            if (buffer == null)
                buffer = new byte[0];

            var outputArray = new byte[newArrayLength + lengthSize]; // amount of bytes for the length bytes
        ...
        private static long CalcNewArrayLength(long currentLength, long blockSize, int lengthSize)
        {
            ...
            while (currentLength % blockSize != blockSize - lengthSize) currentLength++; // TODO: Improve
        ...
        {
            // TODO: Use BigInteger
            var size = appendLengthInBits ? originalLength * 8 : originalLength; // originalLength = length in bytes, i.e. we have to multiply with 8 to convert it into bits
