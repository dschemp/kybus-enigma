...
namespace KybusEnigma.Lib.Hashing.MessageDigest
{
    ...
    {
        protected byte[] PadMd5(byte[] buffer)
        {
            if (buffer == null)
                buffer = new byte[0];
        ...
        private static void AppendLength(byte[] buffer, long originalLength)
        {
            originalLength *= 8; // Bytes to bits
            var lengthBytes = originalLength.Long2BytesArrLittleEndian();
