...
namespace Kybus.Enigma.Hashing
{
    internal static class HashingExtensions
    {
        ...
        {
            int length = (int)Math.Ceiling(arr.Length / 4.0);
            return Enumerable
                .Range(0, length)
        ...
        {
            int length = (int)Math.Ceiling(arr.Length / 8.0);
            return Enumerable
                .Range(0, length)
        ...
        public static byte[] UInt32ArrToUInt8Arr(this uint[] arr)
        {
            ...
            {
                byte[] bytes = UInt32ToUInt8Arr(arr[i]);
        ...
        public static byte[] UInt32ArrToUInt8ArrLE(this uint[] arr)
        {
            ...
            for (int i = 0; i < arr.Length; i++)
            {
                byte[] bytes = UInt32ToUInt8ArrLE(arr[i]);
                Array.Copy(bytes, 0, output, i * 4, bytes.Length);
            }
        ...
        {
            byte[] output = new byte[arr.Length * 8];
